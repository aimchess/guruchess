...
import { 
  ...
  Phone, Globe, Trophy, Award, Clock, Search, MessageSquare, Send
} from "lucide-react"
...
export function BookDemoModal({ isOpen, onClose }: BookDemoModalProps) {
  ...
  const sendToWhatsApp = () => {
    ...
    const message = `*New Demo Request*%0A` +
                    ...
                    `*Schedule:* ${formData.prefDay} @ ${formData.prefTime}`
  ...
  const sendToEmail = async (e: React.FormEvent) => {
    ...
    try {
      await emailjs.send(
        ...
        'template_1elxxot', 
        {
          ...formData, // individual fields for your {{tags}}
      ...
      alert("Email failed. Use WhatsApp instead.")
    } finally {
      setLoading(false)
    }
  }
    ...
    <AnimatePresence>
      {isOpen && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
          ...
          <motion.div initial={{ opacity: 0, scale: 0.95, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95, y: 20 }} className="relative w-full max-w-5xl bg-white rounded-[2.5rem] shadow-2xl overflow-hidden flex flex-col md:flex-row h-full max-h-[95vh]">
            ...
            <div className="flex-1 overflow-y-auto p-8 md:p-12 bg-white">
              {!isSubmitted ? (
                  ...
                  <form className="space-y-4">
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div className="relative">
                            ...
                            <input required name="fullName" value={formData.fullName} onChange={handleInputChange} type="text" placeholder="Player's Full Name" className={inputClass} />
                        </div>
                        <div className="relative">
                            <Calendar className={iconClass} size={18} />
                            <input required name="age" value={formData.age} onChange={handleInputChange} type="text" placeholder="Age or DOB" className={inputClass} />
                        </div>
                    </div>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        ...
                        <div className="relative">
                            ...
                            <input required name="phone" value={formData.phone} onChange={handleInputChange} type="tel" placeholder="WhatsApp Number" className={inputClass} />
                        </div>
                    </div>
                            ...
                            {showCountryList && filteredCountries.length > 0 && (
                                <div className="absolute z-50 w-full mt-1 bg-white border border-slate-100 rounded-xl shadow-xl max-h-40 overflow-y-auto">
                                    {filteredCountries.map(c => (
                                        <div key={c} className="px-4 py-2 hover:bg-slate-50 cursor-pointer text-sm font-medium" onClick={() => { setFormData({...formData, country: c}); setShowCountryList(false) }}>{c}</div>
                                    ))}
                                </div>
                            )}
                        </div>
                        <input name="state" value={formData.state} onChange={handleInputChange} type="text" placeholder="State (Opt)" className={`${inputClass} !pl-5`} />
                        <input name="city" value={formData.city} onChange={handleInputChange} type="text" placeholder="City (Opt)" className={`${inputClass} !pl-5`} />
                    </div>
                    ...
                    <div className="pt-2">
                        ...
                        <div className="relative mb-4">
                            ...
                            <input name="rating" value={formData.rating} onChange={handleInputChange} type="text" placeholder="Current FIDE Rating (Optional)" className={inputClass} />
                        </div>
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            <div className="relative">
                                <Search className={iconClass} size={18} />
                                <input name="lichess" value={formData.lichess} onChange={handleInputChange} type="text" placeholder="Lichess ID" className={inputClass} />
                            </div>
                            <div className="relative">
                                <Search className={iconClass} size={18} />
                                <input name="chesscom" value={formData.chesscom} onChange={handleInputChange} type="text" placeholder="Chess.com ID" className={inputClass} />
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="relative">
                        ...
                        <select name="tournament" value={formData.tournament} onChange={handleInputChange} required className={`${inputClass} appearance-none`}>
                    ...
                    <div className="pt-2">
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            <div className="relative">
                                <Calendar className={iconClass} size={18} />
                                <select name="prefDay" value={formData.prefDay} onChange={handleInputChange} required className={`${inputClass} appearance-none`}>
                            ...
                            <div className="relative">
                                ...
                                <input required name="prefTime" value={formData.prefTime} onChange={handleInputChange} type="text" placeholder="Preferred Time (e.g. 5PM)" className={inputClass} />
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-2">
                        <Button 
                          ...
                          className="w-full h-14 rounded-2xl text-[10px] font-black uppercase tracking-[0.2em] shadow-xl bg-green-600 hover:bg-green-700 text-white transition-all hover:scale-[1.01]"
                        >
                        ...
                        <Button 
                          ...
                          disabled={loading}
                          onClick={sendToEmail}
                          className="w-full h-14 rounded-2xl text-[10px] font-black uppercase tracking-[0.2em] shadow-xl transition-all hover:scale-[1.01]"
                          style={{ backgroundColor: brandOrange, color: 'white' }}
                        >
                          {loading ? "Sending..." : <><Send className="mr-2 h-4 w-4" /> Email Submission</>}
                        </Button>
                    </div>
                  </form>
                </motion.div>
                ...
                <div className="h-full flex flex-col items-center justify-center text-center py-20">
                    ...
                    <p className="text-slate-500 text-sm max-w-xs mt-2 mb-8">We have received your profile. Our head coach will message you soon.</p>
